...
**Most importantly, modifying reactive variables trigger updates of both every query that depends on a reactive variable, as well as every stateful value returned from the useReactiveVariable custom React Hook.** A query depends on a reactive variable if any of the query's requested fields defines a [`read` function](../caching/cache-field-behavior/#the-read-function) that reads the variable's value.
